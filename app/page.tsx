...
export default function Home() {
  ...
  useEffect(() => {
    const fetchInterpretations = async () => {
      ...
      try {
        ...
        setError("Failed to load interpretations, please try again");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setInterpretations((preInterpretations) =>
        preInterpretations?.filter((i) => i.$id !== id)
      );
    } catch (error) {
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="font-bold">
          {" "}
          There is no interpretations, please add new one!
        </div>
      )}
    </div>
  );
